...
* (24 Feb 2017) Add test vectors for hardened derivation with leading zeros
* (4 Nov 2020) Add NEW test vectors for hardened derivation with leading zeros
...
* HD Wallets
  * == wallets /
    ...
    * PARTIALLY shared 
      * can NOT spend coins
  * ' features
    * ❌NOT ALL are -- required by -- supporting clients❌ 
  * uses
    * interchanged BETWEEN DIFFERENT clients (TODO: ❓)

* specification
  * == first part & second part
    * first part
      * FROM single seed -- derive -- tree of keypairs
    * second part
      * how to build a wallet structure | tree
...
* Bitcoin reference client
  ...
  * allows
    * | AFTER ALL transaction,
      * avoid a backup
        * Reason:🧠by default, 100 keys are cached | pool of reserve keys🧠

* HD Wallets
  * STILL, 
    * ❌NOT intended to be
      * shared
      * used | SEVERAL systems SIMULTANEOUSLY❌
  * support
    * hide -- , by using the wallet encrypt feature, -- their private keys
      * if that's the case ->can NOT generate public keys
  * ❌NOT require❌
    * frequent backups
      * Reason: 🧠you can calculate -- , via elliptic curve mathematics permit schemes, the -- public keys / WITHOUT revealing the private keys🧠
      * _Example:_
        * webserver -- generate -- fresh addresses (public key hashes) / EACH customer's address 
          * WITHOUT giving the webserver access -- to the -- corresponding private keys (required for spending the received funds)
  * (NORMALLY) == 1 "chain" of keypairs
    * share a wallet == all-or-nothing basis
      * Reason: 🧠1 chain 🧠
    * use cases
      * 1! wants some (public) keys -- to be -- shared & recoverable
        * _Example:_ webserver
          * does NOT need access -- to -- ALL merchant's wallet's public keys 
          * ONLY needs access -- to -- those addresses / used to receive customers' payments
  * allow
    * selective sharing / derived -- from -- 1! root
      * Reason: 🧠support MULTIPLE keypair chains🧠 
...
* elliptic curve cryptography + [secp256k1](http://www.secg.org/sec2-v2.pdf)'s field and curve parameters 
  * == public key cryptography / used | Bitcoin 
  
* variables / used | this text
  * n
    * == integers / modulo == order of the curve 
  * coordinates of points | curve
  * byte sequences

* addition (+) of 2 coordinate pair
  * == EC group operation's application
* concatenation (||)
  * == append 1 byte sequence | another

* standard conversion functions
  * point(p)
    * == coordinate pair /
      * (EC point multiplication (== repeated EC group operation's application) (secp256k1(integer p)) )'s output
  * ser<sub>32</sub>(i)
    * serialize (32-bit unsigned integer i) -- as a -- 4-byte sequence / 
      * FIRSTLY, MOST significant byte
  * ser<sub>256</sub>(p)
    * serialize(integer p) -- as a -- 32-byte sequence
      * FIRSTLY, MOST significant byte
  * ser<sub>P</sub>(P)
    * serializes (P) -- , via SEC1's compressed form, as a -- byte sequence 
      * P = (x,y)
      * SEC1's compressed form
        * (0x02 or 0x03) || ser<sub>256</sub>(x)
...
* function
  * from a parent key -- derives a -- number of child keys 
    * private and public keys are chain code extended identically
      * Reason:🧠NOT depend ONLY on the key itself🧠 

* chain code
  * := extension -- with -- 256 bits of entropy
  * ExampleS: let's have k: normal private key, c: chain code, K = point(k) 
    * (k, c)
...
* / EACH child keys
  * an index
    * normal child keys's indices: [0, 2<sup>31</sup>-1]
    * hardened child keys' indices: [2<sup>31</sup>, 2<sup>32</sup>-1]

* i<sub>H</sub>
  * := i+2<sup>31</sup>
    * Reason:🧠ease notation -- for -- hardened key indices🧠 a number 
...
* In case parse<sub>256</sub>(I<sub>L</sub>) ≥ n or k<sub>i</sub> = 0, the resulting key is invalid, and one should proceed with the next value for i
...
The function CKDpub((K<sub>par</sub>, c<sub>par</sub>), i) &rarr; (K<sub>i</sub>, c<sub>i</sub>) computes a child extended public key from the parent extended public key
...
* wallet structure | key tree

## The default wallet layout

* recommendations
  * Bitcoin clients mimic it

* HDW
  * 💡organized -- as -- MULTIPLE numbered 'accounts'💡
    * default account
      * := account / number 0 
    * account == 2 keypair chains
      * internal keypair chain
        * uses
          * ALL other operations (change addresses, generation addresses, ..., anything that doesn't need to be communicated)
      * external keypair chain
        * uses
          * generate NEW public addresses
      * _Examples:_
        * m/i<sub>H</sub>/0/k
          * == HDW derived from master m's account number i's external chain's k'th keypair  
            * external == 0
        * m/i<sub>H</sub>/1/k 
          * == HDW derived from master m's account number i's internal chain's k'th keypair
            * internal == 1
    * clients 
      * ❌do NOT need to support > 1 account❌
      * if they 
        * ONLY use 1 account -> default account
        * do NOT support separate keychains -> they should use external keypair chain for ALL

## Use cases

### Full wallet sharing: m

In cases where two systems need to access a single shared wallet, and both need to be able to perform spendings, one needs to share the master private extended key
...
* An extra look-ahead could be active for the first unused account's chains - triggering the creation of a new account when used
...
* This will allow the headquarters to maintain a super-wallet that sees all incoming and outgoing transactions of all offices, and even permit moving money between the offices.
...
When an unsecure webserver is used to run an e-commerce site, it needs to know public addresses that are used to receive payments
* The webserver only needs to know the public extended key of the external chain of a single account
* This means someone illegally obtaining access to the webserver can at most see all incoming payments but will not be able to steal the money, will not (trivially) be able to distinguish outgoing transactions, nor be able to see payments received by other webservers if there are several.
...
* Gregory Maxwell
  * original idea of type-2 deterministic wallets
* Alan Reiner
  * implement this scheme | Armory
* Eric Lombrozo
  * review this BIP
* Mike Caldwell
  * version bytes -- to obtain -- human-recognizable Base58 strings
