...
* NEW "standard" transaction type | Bitcoin scripting system + ADDITIONAL validation rules | NEW transactions

# Motivation

* pay-to-script-hash
  * 's goal
    * 👀modify the supplier of the conditions / redeem a transaction: FROM the sender -- to the -- redeemer👀

* allow
  * sender can fund any arbitrary transaction
...
* `OP_HASH160 [20-byte-hash-value] OP_EQUAL`
  * 💡NEW standard transaction type / relayed & included | mined blocks💡
  * `[20-byte-hash-value]` 
    * == `push-20-bytes-onto-the-stack` opcode (0x14) + 20 bytes
  * if you want to redeem -- via -- `...signatures... {serialized script}`
    * == standard `scriptSig`

* `serialized script` OR `redeemScript`

* transactions / redeem these pay-to-script outpoints
  * ONLY standard := when the `serialized script` is itself == one of the other standard transaction types 

* rules for validating these outpoints | relay the transactions OR considering them for inclusion | new block

    ```c++
    # if there are operations != "push data" operations | `scriptSig` -> validation fails 
    # normal validation steps
    #   FROM signatures & {serialized script}, create initial stack
    #   compute the script's hash
    #       if it != outpoint's hash -> validation fails immediately  
    # steps
    #   {serialized script} is popped off | initial stack
    #   transaction is validated again -- against -- the popped stack 
    #   deserialize the script -- as the -- scriptPubKey
    ```

These new rules should only be applied when validating transactions in blocks with timestamps >= 1333238400 (Apr 1 2012) <ref>[https://github.com/bitcoin/bitcoin/commit/8f188ece3c82c4cf5d52a3363e7643c23169c0ff Remove -bip16 and -paytoscripthashtime command-line arguments]</ref>
There are transactions earlier than 1333238400 in the block chain that fail these new validation rules. <ref>[https://web.archive.org/web/20141122040355/http://blockexplorer.com/tx/6a26d2ecb67f27d1fa5524763b49029d7106e91e3cc05743073461a719776192 Transaction 6a26d2ecb67f27d1fa5524763b49029d7106e91e3cc05743073461a719776192]</ref>
