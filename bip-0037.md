...
* goal
  * adds support to the P2P protocol /
    * allows
      * peers can reduce the amount of transaction data / they are sent ->
        * chain synchronization fast, cheap
        * run | old phones / limited memory 

* Peers
  * | AFTER completing the version handshake,
    * can set filters / EACH connection 

* filter
  * := [Bloom filter](http://en.wikipedia.org/wiki/Bloom_filter) | data / derived from transactions
...
* amount of bandwidth / needed to download blocks & transaction broadcasts: grows
  * Reason:🧠Bitcoin usage grows🧠
* Clients / implement 'simplified payment verification'
  * ❌do NOT try to FULLY verify the blockchain ❌
  * ONLY check that block headers connect together correctly
    * -> trust that: transactions | chain / high difficulty: are valid

* [https://bitcoin.org/en/developer-guide#simplified-payment-verification-spv|SPV] clients
  * cons
    * slows down user synchronization process
    * wastes users bandwidth
    * increases memory usage 
  * Reason of the cons:🧠require
    * download ALL contents of blocks + ALL broadcast transactions
    * throw away MOSTLY ALL transactions / NOT relevant to their wallets🧠
  * impact of the cons
    * user Android "Bitcoin Wallet" app complaints / implements SPV mode

# Design rationale

The most obvious way to implement the stated goal would be for clients to upload lists of their keys to the remote node
...
* if you want to check data was inserted by the client -> test the filter against arbitrary pieces of data 
  * question: of what pieces of data should be inserted/tested?

* algorithm / determine if a transaction matches -- the -- filter

    ```c++
    # Test the transaction's hash itself
    # Test output script's EACH data element / EACH output
    #   == EACH output script's hash & key are tested INDEPENDENTLY
    #   if an output matches | testing a transaction -> the node might need -- , by inserting the serialized `COutPoint` structure, -- to update the filter 
    # test the serialized `COutPoint` structure / EACH input
    # test input script's EACH data element / EACH input
    #   input scripts ONLY contain data elements
    # Otherwise there is NO match
    ```
  * ⚠️| find the match -> the algorithm aborts⚠️

* TODO: In this way addresses, keys and script hashes (for P2SH outputs) can all be added to the filter. You can also match against classes of transactions that are marked with well known data elements in either inputs or outputs, for example, to implement various forms of [[https://en.bitcoin.it/wiki/Smart_Property|Smart property]].
