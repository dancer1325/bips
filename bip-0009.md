...
* goal
  * change the semantics of Bitcoin blocks' 'version' field /
    * allow MULTIPLE backward-compatible changes ("soft forks") / can be deployed in parallel
      * soft fork votes is tailed / EACH retarget period
      * AFTER consensus change succeeds OR times out ->
        * "fallow" pause | 
          * NOT used
            * Reason:🧠identify bugs🧠
        * AFTERWARDS,
          * it can be reused
    * 💡version field == bit vector💡 / 
      * EACH bit can be used -- to -- track an independent change

# Motivation

* [BIP 34](bip-0034.md)
  * == mechanism / does soft-forking changes
    * WITHOUT a predefined flag timestamp (OR flag block height)
    * rely on miner support / indicated by a higher version number | block headers
* As it relies on comparing version numbers as integers however, 
it only supports one single change being rolled out at once, 
requiring coordination between proposals, 
and does not allow for permanent rejection: as long as one soft fork is not fully rolled out, no future one can be scheduled.
...
(all negative numbers, as nVersion is interpreted as a signed integer, as well as 0 and 1)
* downside
  * / EACH upgrade,
    * permanently restricts the set of allowed nVersion field values
* | BIP 65 & BIP 66,
  * approach reused
  * removed nVersions 2 and 3 -- as -- valid options
    * Reason:🧠unnecessary🧠
