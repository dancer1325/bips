...
* [BIP 34](bip-0034.md)
  * == mechanism / does soft-forking changes
    ...
    * rely on miner support / based on higher version number | block headers
  * cons
    * ONLY supports 1! change being rolled out / time
      * Reason:🧠based on compare version numbers🧠 
      * -> require coordination BETWEEN proposals
    * if you want to schedule a NEW one -> PREVIOUS one MUST be fully rolled out
    * removed 2<sup>31</sup>+2 values (ALL negative numbers, 0 and 1) | set of valid version numbers
      * Reason:🧠
        * BEFORE getting 95%,
          * miners can choose v1 OR v2
        * AFTER getting 95%,
          * ALL miners ONLY accept `nVersion>=2` blocks🧠
